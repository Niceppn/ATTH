...
function App() {
  ...
  const [currentPage, setCurrentPage] = useState("testcase"); // 'testcase', 'management', 'reports', or 'testresults'
  ...
  const handleNavigation = (page) => {
    ...
    if (page === "testcase") {
      setShowTestCases(false); // Reset to project selection when navigating to test case page
    }
  };
  ...
  const renderCurrentPage = () => {
    switch (currentPage) {
      case "testcase":
        if (showTestCases && selectedProject) {
          return (
            <TestCaseTable
              testCases={testCases}
    ...
    <div className="app-layout">
      ...
      <div className="main-wrapper">
        ...
        <main className="main-content">
          <section className="dashboard-section">{renderCurrentPage()}</section>
        </main>
      </div>
    </div>
  );
